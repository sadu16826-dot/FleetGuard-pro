...
export function VehicleTable({ vehicles }: { vehicles: VehicleListItem[] }) {
  ...
  return <>
    ...
    <Card className="overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3 text-xs text-slate-500"><span>Showing {filtered.length} of {vehicles.length} active vehicles</span><div className="flex items-center gap-4"><button type="button" onClick={() => { const params = new URLSearchParams(searchParams.toString()); if (params.get("includeInactive") === "1") params.delete("includeInactive"); else params.set("includeInactive", "1"); router.push(`/vehicles${params.toString() ? `?${params}` : ""}`); }} className="font-semibold text-blue-600">{searchParams.get("includeInactive") === "1" ? "Hide deactivated" : "Show deactivated"}</button><span className="flex items-center gap-1">Sort: Last updated <ChevronDown size={13}/></span></div></div>
    </Card>
