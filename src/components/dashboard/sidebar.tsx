...
import { allowedModules, roleLabel } from "@/lib/permissions";

export function Sidebar({ open, onClose, user }: { open: boolean; onClose: () => void; user: { name: string; role: UserRole } }) {
  const pathname = usePathname();
  const navigation = dashboardNavigation.filter((item) => allowedModules(user.role).has(item.module));
  const [expanded, setExpanded] = useState<string[]>(() => navigation.filter(item => item.children && pathname.startsWith(item.href)).map(item => item.label));
