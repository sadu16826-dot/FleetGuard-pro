...
export async function POST(
  ...
  try {
    ...
    if (!Number.isFinite(endingKm) || endingKm < (trip.startKm ?? 0))
      return NextResponse.json(
        {
          message:
    ...
    const hasSafetyIssue =
      newDamage || accident || tyreProblem || warningLight || mechanicalProblem;
    const vehicleStatus = checked(form, "notRoadworthy")
      ? "NOT_ROADWORTHY"
      : hasSafetyIssue
        ? "INSPECTION_REQUIRED"
        : "AVAILABLE";
    ...
    const returned = await db.$transaction(async (tx) => {
      const completed = await tx.trip.updateMany({
        ...
        data: {
          ...
          returnBatteryPercentage: batteryPercentage,
          newDamage,
          accidentReported: accident,
          tyreProblem,
          warningLight,
          mechanicalProblem,
      ...
      await tx.tripVehiclePhoto.createMany({
      ...
      const preTripPhotoCount = await tx.tripVehiclePhoto.count({
        where: { tripId, vehicleId: id, phase: "PRE_TRIP" },
      });
      await tx.inspection.create({
        data: {
          tripId,
          inspectionType: "POST_TRIP",
          tyreStatus: tyreProblem ? "ATTENTION" : "OK",
          damageStatus: newDamage ? "NEW_DAMAGE" : "CLEAR",
      ...
      await tx.vehicleActivity.create({
        data: {
          ...
          metadata: {
            ...
            preTripPhotoCount,
            postTripPhotoCount: photos.length,
          },
        },
      });
    ...
    if (process.env.NODE_ENV !== "production")
      console.error("Return trip failed", error);
    ...
    if (error instanceof Error && error.message === "TRIP_ALREADY_RETURNED")
      return NextResponse.json(
        { message: "This trip has already been returned." },
        { status: 409 },
      );
    return accessFailure(
      error,
      "Vehicle return could not be completed. No changes were saved.",
    );
  }
}
