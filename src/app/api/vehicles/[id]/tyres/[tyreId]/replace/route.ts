...
export async function POST(
  ...
  try {
    ...
    const old = await db.tyre.findFirst({
      where: { id: tyreId, vehicleId: id, status: { not: "REPLACED" } },
    });
    if (!old)
      return NextResponse.json(
        { message: "Tyre could not be found." },
        { status: 404 },
      );
    const km = Number(input.replacementKm);
    if (
      !input.brand ||
      !input.replacementDate ||
      !Number.isFinite(km) ||
      km < Number(old.installationKm ?? 0) ||
      km > vehicle.currentKm
    )
      return NextResponse.json(
        {
          message:
            "Brand, replacement date and a replacement KM within the vehicle odometer range are required.",
        },
        { status: 400 },
      );
    const tyre = await db.$transaction(async (tx) => {
      ...
      await tx.tyreHistory.create({
        data: {
          tyreId,
          action: "REPLACED",
          description: `Tyre replaced at ${km} KM`,
          metadata: { replacementCost: input.cost },
        },
      });
      const created = await tx.tyre.create({
        data: {
          ...
          installationDate: new Date(input.replacementDate!),
          installationKm: km,
      ...
      await tx.vehicleActivity.create({
        data: {
          ...
          description: `Tyre replaced at ${old.position}`,
          metadata: { oldTyreId: tyreId, newTyreId: created.id },
        },
      });
      return created;
    });
    ...
    if (process.env.NODE_ENV !== "production")
      console.error("Replace tyre failed", error);
    return accessFailure(error, "Unable to replace tyre.");
  }
}
