...
const routeModules: Record<string, PermissionModule> = {
  fuel: "FUEL",
  documents: "DOCUMENTS",
  accidents: "ACCIDENTS",
  trips: "TRIPS",
...
export default async function ModulePage({ params }: { params: Promise<{ section: string[] }> }) {
  ...
  const permissionModule = routeModules[section[0]];
  if (permissionModule) await requirePermission(permissionModule);
